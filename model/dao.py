...
class DAO:
    """Data Access Object singleton class
    for connecting with the database"""
    _instance = None

    def __init__(self, conn):
        """Singleton instance initialization
        with the parameter set as the connection url"""
    ...
    def initializedb(self):
        """Creating the HISTORY table in the database
         if it doesn't already exist"""
        (self._cursor.
            execute("""CREATE TABLE IF NOT EXISTS HISTORY (
                id INTEGER NOT NULL constraint
                history_pk primary key autoincrement,
                amount INT,
                before INT,
                after INT,
                date TEXT,
    ...
    def addentry(self, entry):
        """Adding the given parameter entry
        to the HISTORY table in the database"""
        self._cursor.execute("""
        INSERT INTO HISTORY (amount, before, after, date, wallet, reason)
        VALUES (?, ?, ?, ?, ?, ?)
        """, (entry.amount, entry.before, entry.after,
              entry.date, entry.wallet, entry.reason))
        self._connection.commit()
    ...
    def listentries(self):
        ...
        res = (self._cursor
               .execute("SELECT wallet, amount, before,"
                        " after, date, reason FROM HISTORY;")
               .fetchall())
    ...
    def importhistory(self, entrylist: list):
        ...
        for entry in entrylist:
            command += \
                (f"\nINSERT INTO HISTORY (wallet, amount, before,"
                 f" after, date, reason) "
